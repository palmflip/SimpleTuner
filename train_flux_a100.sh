...
# Настройки для A100 SXM - оптимальная производительность
export CUDA_VISIBLE_DEVICES=0
export TORCH_CUDA_ARCH_LIST="8.0"  # A100 поддерживает CUDA 8.0
export CUDA_LAUNCH_BLOCKING=0
export NCCL_P2P_DISABLE=0
export NCCL_IB_DISABLE=0

# Оптимизации CUDA для A100
export TORCH_CUDNN_USE_HEURISTIC_MODE_B=1
export TORCH_CUDNN_V8_API_ENABLED=1
export PYTORCH_CUDA_ALLOC_CONF="max_split_size_mb:256"  # Меньше для A100
...
echo "🔍 Проверяем структуру проекта для A100 SXM..."
...
echo "🚀 Запускаем тренировку Flux LoRA на A100 SXM..."
echo "⏱️  Примерное время тренировки: $((TOTAL_STEPS / 45)) минут"  # A100 медленнее H100

# Обновляем количество шагов в конфиге
sed -i "s/\"--max_train_steps\": [0-9]*/\"--max_train_steps\": $TOTAL_STEPS/" config/config_a100.json

# Логируем параметры
echo "📋 Параметры тренировки для A100 SXM:"
echo "   - Batch Size: 4 (оптимально для A100)"
...
echo "   - VRAM: ~35-38GB (A100 40GB/80GB)"

# Запускаем тренировку
python train.py \
    --config=config/config_a100.json \
    2>&1 | tee logs/training_a100_$(date +%Y%m%d_%H%M%S).log
